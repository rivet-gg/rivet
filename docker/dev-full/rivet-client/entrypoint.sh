...
RIVET_SERVER_IPV4="192.168.100.1"

# Create admin chain that only accepts traffic from the GG subnet
# PUBLIC_IFACE="eth0"
# VLAN_IFACE="eth1"
# GG_VLAN_SUBNET="0.0.0.0/0"

# # MARK: Linux Traffic Control
# for iface in $PUBLIC_IFACE $VLAN_IFACE; do
...
for ipt in iptables ip6tables; do
	...
	add_ipt_chain "$ipt" "nat" "$NAT_CHAIN"

	# Jump to RIVET-NAT chain from OUTPUT (for locally generated traffic)
	# and PREROUTING (for container traffic)
	#
	# CNI firewall plugin will automatically jump to $ADMIN_CHAIN on "filter"
	add_ipt_rule "$ipt" "nat" "OUTPUT" "-j $NAT_CHAIN"
	add_ipt_rule "$ipt" "nat" "PREROUTING" "-j $NAT_CHAIN"

	# TODO: This IP will change
	# MARK: Expose rivet-server inside the container at 192.168.100.1
	# Only applicable to IPv4 (assuming Docker networking stack is IPv4 only)
	if [ "$ipt" == "iptables" ]; then
	...
	# if [ "$ipt" == "iptables" ]; then
	# 	# MARK: GG TOS
	# 	add_ipt_rule "$ipt" "mangle" "RIVET-FORWARD" "-s $GG_VLAN_SUBNET -d $SUBNET_VAR -j RIVET-TOS-GG"
	# 	add_ipt_rule "$ipt" "mangle" "RIVET-FORWARD" "-s $SUBNET_VAR -d $GG_VLAN_SUBNET -j RIVET-TOS-GG"
	#
	# 	# MARK: GG ingress
	# 	# Prioritize traffic
	# 	add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-s $GG_VLAN_SUBNET -d $SUBNET_VAR -j MARK --set-mark 1"
	# 	# Accept traffic
	# 	add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-s $GG_VLAN_SUBNET -d $SUBNET_VAR -j ACCEPT"
	#
	# 	# MARK: GG egress
	...
	# # MARK: Public egress
	# # Prioritize traffic
	# add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-s $SUBNET_VAR -o $PUBLIC_IFACE -j MARK --set-mark 1"
	#    # Allow egress traffic
	# add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-s $SUBNET_VAR -o $PUBLIC_IFACE -j ACCEPT"
	#
	# # Allow public ingress traffic on all ports because this is already mapped by CNI
	# add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-p tcp -d $SUBNET_VAR -i $PUBLIC_IFACE -j ACCEPT"
	# add_ipt_rule "$ipt" "filter" "$ADMIN_CHAIN" "-p udp -d $SUBNET_VAR -i $PUBLIC_IFACE -j ACCEPT"
	#
	# # MARK: Deny

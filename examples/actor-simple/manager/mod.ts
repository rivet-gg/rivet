...
import { GetOrCreateRequest, queryActor } from "./query.ts";
import {
	PORT_NAME,
	RivetEnvironment,
} from "../../../sdks/actors/client/src/utils.ts";
...
export type ActorsRequest = GetOrCreateRequest;
...
app.post("/actors", async (c: HonoContext) => {
	...
	const actor = await queryActor(rivetClient, environment, { getOrCreate: body });

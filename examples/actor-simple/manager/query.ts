...
import { assertUnreachable, PORT_NAME, RivetEnvironment, Tags } from "./utils.ts";
...
export interface GetOrCreateRequest { tags: Tags, create?: CreateRequest }
...
export async function queryActor(
	...
	if ("actorId" in query) {
		...
		const getActor = await getWithTags(client, environment, tags as Tags);
		if (getActor) {
			return getActor;
		} else {

...
if (!userCodeFileName) {
	console.error(
		"ERROR: USER_CODE_FILE_NAME environment variable is required",
	);
	Deno.exit(1);
}
 ...
 */
async function loadAndRunUserCode(): Promise<{
	success: boolean;
	output: string;
}> {
	try {
...
const port = Number.parseInt(Deno.env.get("PORT_HTTP") || "8000");

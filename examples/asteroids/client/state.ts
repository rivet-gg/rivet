...
import { ActorHandle, Client } from "actor-core/client";
...
export default interface GameClient {
	...
	actorCore: Client;
	matchmaker: ActorHandle<Matchmaker>;
}
...
export async function setup(client: GameClient) {
	...
	const res = await client.matchmaker.findLobby({
		//query: { version: "local", tags: { gameMode } },
		query: { version: "local" },
		players: [{}],
		noWait: true,
	});
	if (!("localDevelopment" in res.lobby.backend)) throw new Error("UNIMPLEMENTED");
	const port = res.lobby.backend.localDevelopment.ports.websocket;
	if (!port) throw new Error("Missing port");
	if (!res.players[0]) throw new Error("Missing player");

	client.lobbyAddress = `${port.protocol}://${port.hostname}:${port.port}`
	client.playerToken = res.players[0].token;

	client.connection = createConnection(client.lobbyAddress, client.playerToken, client.performance.now());

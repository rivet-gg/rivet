...
const TAB_GROUPS: TabGroup[] = [
	{
		...
		items: [
			...
			},
			{
				...
				description: "Execute untrusted code securely in isolated environments with resource limitations and security controls.",
	...
	},
	{
		...
		items: [
			...
			},
			{
				...
				icon: faCourtSport,
				title: "Live Events",
				description: "Create interactive live events with realtime participation for thousands of concurrent users.",
				image: imgLiveEvents,
				docs: {
					actors: true,
					containers: false,
		...
		items: [
			{
				...
				description: "Isolate each customer's data in dedicated database instances for improved security and performance.",
			...
			},
			{
				...
				icon: faOven,
				title: "Batch Jobs",
				description: "Process large datasets in parallel with efficient resource allocation and automatic scaling.",
				image: imgBatchJobs,
				docs: {
					actors: true,
					containers: false,
		...
		items: [
			{
				...
				title: "ETL Pipeline",
				description: "Build data pipelines that extract, transform, and load data between various sources and destinations.",
...
const DocLink = ({ type, activeTab }: { type: string; activeTab: string }) => (
	<a 
		href={`/docs/${activeTab}/${type.toLowerCase()}`} 
	...
	>
		{type}<span className="ml-1.5 text-white/50">→</span>
	</a>
);
					...
					<div className="w-full lg:w-1/2 p-5">
						...
						<div className="bg-zinc-900 rounded-xl overflow-hidden border border-zinc-800">
							...
							<div className="bg-[#020202]">
								...
								<div className="p-5">
									<h3 className="text-xl font-medium text-white mb-3">{activeTabData.title}</h3>
								...
								<div className="p-10 flex items-center justify-center min-h-[250px]">
									<Image 
										...
										width={activeTabData.image.width ? activeTabData.image.width / 2 : undefined}
										height={activeTabData.image.height ? activeTabData.image.height / 2 : undefined}
									/>
								</div>
							</div>
							...
							{activeTabData.docs && (
								<div className="p-4">
									<div className="text-sm font-medium text-white/90 mb-2">Documentation</div>
									<div className="flex gap-3">
										{activeTabData.docs.actors && <DocLink type="Actors" activeTab={activeTab} />}
										{activeTabData.docs.containers && <DocLink type="Containers" activeTab={activeTab} />}
										{activeTabData.docs.functions && <DocLink type="Functions" activeTab={activeTab} />}
									</div>
								</div>
							)}
						</div>

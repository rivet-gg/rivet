...
On the other hand, **stateful serverless provides an infinitely running process with storage**. An example implementation on [RivetKit](https://rivetkit.org/introduction) would look like this:
...
We encourage developers building on Rivet to use [RivetKit](https://github.com/rivet-gg/rivetkit). However, we also provide a [low-level API](https://rivet.gg/docs/javascript-runtime) for defining actors:
...
The `onmessage` handler is very similar to the `fetch` handler in a Durable Object or an [Action](https://rivetkit.org/concepts/actions) in RivetKit.
...
Once a general specification is implemented, a reference specification is trivial to implement on any of the providers previously mentioned. A compatibility layer could be implemented on the existing work with [RivetKit drivers](https://rivetkit.org/drivers/overview).

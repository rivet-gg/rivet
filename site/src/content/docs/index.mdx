...
Rivet is the platform to build realtime, edge, or agent applications that is scalable, stateful, and serverless. Build without the limitations of Redis or timeouts of Lambda. At the core of Rivet provides a primative -- Actors. Rivet Actors are built with built-in support for remote procedure calls, state, events, and edge networking.

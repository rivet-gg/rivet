...
Tags are simple key-value pairs (like labels) attached to every actor. Tags serve two purposes:

1. **Actor Discovery**: Find specific actors using [`client.get(tags, opts)`](https://jsr.io/@rivet-gg/actor-client/doc/~/Client.prototype.get) (see below)
2. **Organization**: Group actors for monitoring & administration

When structuring tags, keep values short, use consistent naming patterns, and avoid storing sensitive information.

The `name` tag is special. This identifies what type of actor to spawn.
...
To get or create an actor, use [`client.get(tags)`](https://jsr.io/@rivet-gg/actor-client/doc/~/Client.prototype.get). This will attempt to find an actor matching a subset of the provided tags or create an actor if not found.
...
To explicitly create a new actor, use [`client.create(tags)`](https://jsr.io/@rivet-gg/actor-client/doc/~/Client.prototype.create). This will create a new actor with the provided tags.
...
Each actor has a unique identifier that can be used instead of tags. To get an actor by its ID, use [`client.getWithId(id)`](https://jsr.io/@rivet-gg/actor-client/doc/~/Client.prototype.getWithId).
...
When creating an actor, you can specify additional tags beyond those provided in the first argument. If `opts.create.tags` is not provided, the tags from the query will be used. For example:
...
const actor = client.get<MyDocument>(
  ...
  {
    create: {
      // Tags assigned to the created actor
      tags: {
        name: 'my_document',
...
By default, actors will choose the optimal region based on the client's latency. The can be overridden by defining `create.region`:

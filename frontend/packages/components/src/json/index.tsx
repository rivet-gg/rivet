...
function Value({
  ...
  if (typeof value === "string") {
    return (
      <StringValue path={path} value={value} editable={editable} {...props} />
    );
  }
  if (typeof value === "number") {
    return (
      <NumberValue path={path} value={value} editable={editable} {...props} />
    );
  }
  if (typeof value === "boolean") {
    return (
      <BooleanValue path={path} value={value} editable={editable} {...props} />
    );
  }
  if (value === null) {
            ...
            {keys.slice(0, 4).map((key, index, array) => {
              ...
              return (
                <div key={key} className="flex italic">
                  <span className="text-foreground/70 mr-1">{key}:</span>{" "}
                  <Value
                    value={value[key]}
                    path={`${path}.${key}`}
                    editable={false}
...
function Editable({
  ...
  function edit() {
    const alreadyEditing = ref.current?.querySelector('[data-editing="true"]');
    if (!alreadyEditing) {
      setIsEditing(true);
    }
  }

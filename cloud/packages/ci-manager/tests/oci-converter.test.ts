...
async function validateOCIBundle(bundleTarPath: string): Promise<void> {
	...
	try {
		...
		const rootfsStat = await import("fs/promises").then((fs) =>
			fs.stat(rootfsPath),
		);
...
beforeAll(async () => {
	...
	try {
		...
		throw new Error(
			"Docker is not available. Please install Docker to run OCI converter tests.",
		);
	}
	...
	try {
		...
		throw new Error(
			"skopeo is not available. Please install skopeo to run OCI converter tests.",
		);
	}
	...
	try {
		...
		throw new Error(
			"umoci is not available. Please install umoci to run OCI converter tests.",
		);
	}
});
...
test("createTestDockerImage builds and saves Docker image", async () => {
	...
	const stats = await import("fs/promises").then((fs) =>
		fs.stat(dockerTarPath),
	);
	expect(stats.isFile()).toBe(true);
	expect(stats.size).toBeGreaterThan(0);

	console.log(
		`Test Docker image created: ${dockerTarPath} (${stats.size} bytes)`,
	);
}, 60000);
...
test("convertDockerTarToOCIBundle converts Docker tar to OCI bundle", async () => {
	...
	try {
		...
		const stats = await import("fs/promises").then((fs) =>
			fs.stat(result.bundleTarPath),
		);
		expect(stats.isFile()).toBe(true);
		expect(stats.size).toBeGreaterThan(0);

		console.log(
			`OCI bundle created: ${result.bundleTarPath} (${stats.size} bytes)`,
		);
...
test("full workflow: Docker image -> direct OCI bundle conversion", async () => {
	...
	try {
		...
		const stats = await import("fs/promises").then((fs) =>
			fs.stat(convertResult.bundleTarPath),
		);
		expect(stats.isFile()).toBe(true);
		expect(stats.size).toBeGreaterThan(0);
		...
		console.log(
			`Full workflow completed successfully: ${convertResult.bundleTarPath}`,
		);
	} finally {

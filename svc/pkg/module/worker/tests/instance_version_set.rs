...
async fn instance_version_set_fly(ctx: TestCtx) {
	...
	// Update version
	{
		...
		msg!([ctx] module::msg::version_create(version_id) -> module::msg::version_create_complete {
			...
			image: Some(module::msg::version_create::message::Image::Docker(module::msg::version_create::message::Docker {
				image_tag: "ghcr.io/rivet-gg/rivet-module-hello-world:0.0.2".into(),
			})),
		})
		.await
		.unwrap();

		msg!([ctx] module::msg::instance_version_set(instance_id) -> module::msg::instance_version_set_complete {
			instance_id: Some(instance_id.into()),
			version_id: Some(version_id.into()),
		})
		.await

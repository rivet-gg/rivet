...
pub async fn run_from_env(
	...
	for team in &teams.teams {
		...
		let distinct_id = build_distinct_id(ctx.config(), format!("team:{team_id}"));
	...
	for game in &games.games {
		...
		let distinct_id = build_distinct_id(ctx.config(), format!("game:{game_id}"));
	...
	for ns in &namespaces.namespaces {
		...
		let distinct_id = build_distinct_id(ctx.config(), format!("ns:{ns_id}"));
...
fn build_distinct_id(config: &rivet_config::Config, entity: impl Display) -> String {
	format!(
		"cluster:{}:{}:{entity}",
		config.rivet.namespace, config.rivet.cluster_id
	)
}

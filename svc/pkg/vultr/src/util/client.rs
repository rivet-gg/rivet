...
impl Client {
	pub async fn new(api_token: Option<String>) -> GlobalResult<Self> {
		let api_token = if let Some(api_token) = api_token {
			...
			util::env::read_secret(&["vultr", "token"]).await?
		};
	...
	pub async fn get<T: DeserializeOwned>(&self, endpoint: &str) -> GlobalResult<T> {
		let res = self
			.request(
				self.inner
					.get(&format!("https://api.vultr.com/v2{endpoint}")),
				None,
				false,
			)
			.await?;
	...
	pub async fn delete(&self, endpoint: &str) -> GlobalResult<()> {
		self.request(
			self.inner
				.delete(&format!("https://api.vultr.com/v2{endpoint}")),
			None,
			true,
		)
		.await?;
	...
	pub async fn post<T: DeserializeOwned>(
		...
		let res = self
			.request(
				self.inner
					.post(&format!("https://api.vultr.com/v2{endpoint}"))

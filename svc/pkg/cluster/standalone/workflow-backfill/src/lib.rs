		...
		Box::pin(async move {
				...
				bctx.workflow("cluster_datacenter", |wf| {
					...
					wf.activity(
						...
						InsertDbInput {
							...
							provider: unwrap!(cluster::types::Provider::from_repr(
								dc.provider.try_into()?
							)),
							provider_datacenter_id: dc.provider_datacenter_id.clone(),
							provider_api_token: dc.provider_api_token.clone(),
							...
							pools: {
								let proto =
									backend::cluster::Pools::decode(dc.pools.as_slice())?.pools;
							...
							build_delivery_method: unwrap!(
								cluster::types::BuildDeliveryMethod::from_repr(
			...
			for server in server_rows {
				...
				let firewall_preset = match pool_type {
					cluster::types::PoolType::Job | cluster::types::PoolType::Pegboard | cluster::types::PoolType::PegboardIsolate => {
					...
					if let cluster::types::PoolType::Gg = pool_type {
						if let Some(cf) = cf {
							...
							wf.sub_workflow(|swf| {
								...
								if cf.dns_record_id.is_some()
									&& cf.secondary_dns_record_id.is_some()
								{
									#[derive(Serialize, Hash)]
									struct InsertDbInput {
										server_id: Uuid,
									...
									swf.activity(
										...
										InsertDbInput {
											server_id: server.server_id,
											primary_dns_record_id: unwrap!(cf
												.dns_record_id
												.clone()),

...
pub async fn gen_remap() -> GlobalResult<GenRemapS3Output> {
	...
	let endpoint_external = s3_util::s3_endpoint_external("bucket-build")?;
	let region = s3_util::s3_region("bucket-build")?;
	let (access_key_id, secret_access_key) = s3_util::s3_credentials("bucket-build")?;

...
async fn nomad_node_drain_complete(ctx: TestCtx) {
	...
	let mut registered_sub = subscribe!([ctx] nomad::msg::monitor_node_registered(server_id))
		.await
		.unwrap();
	registered_sub.next().await.unwrap();

	let mut destroy_sub = subscribe!([ctx] cluster::msg::server_destroy(server_id))
		.await
		.unwrap();
	...
	// Server should get destroyed after the drain is complete

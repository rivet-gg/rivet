...
use proto::backend::{self, matchmaker::lobby_runtime::NetworkMode as LobbyRuntimeNetworkMode};
use regex::Regex;
use serde_json::json;

use crate::{oci_config, util_job};
...
impl From<backend::dynamic_servers::GameGuardProtocol> for ProxyProtocol {
	fn from(protocol: backend::dynamic_servers::GameGuardProtocol) -> Self {
		match protocol {
			backend::dynamic_servers::GameGuardProtocol::Http => Self::Http,
			backend::dynamic_servers::GameGuardProtocol::Https => Self::Https,
			backend::dynamic_servers::GameGuardProtocol::Tcp => Self::Tcp,
			backend::dynamic_servers::GameGuardProtocol::TcpTls => Self::TcpTls,
			backend::dynamic_servers::GameGuardProtocol::Udp => Self::Udp,
		}
	}
}
...
// 		.iter()
// 		.map(|port| {
// 			let target = unwrap!(port.target_port) as u16;
//
// 			// TODO
...
// 				"Protocol": TransportProtocol::Udp.as_cni_protocol(),
// 			})
// 		})
// 		.collect::<Vec<_>>();
//
// 	// Also see util_mm:consts::DEFAULT_ENV_KEYS
// 	let mut my_env = runtime
// 		.env_vars
// 		.iter()
...
// 			// less confusion.
// 			(format!("PORT_{}", port.label.replace('-', "_")), port_value)
// 		}))
// 		.map(|(k, v)| format!("{k}={v}"))
// 		.collect::<Vec<String>>();
// 	my_env.sort();
// 	tracing::info!(?my_env, "my env");
//

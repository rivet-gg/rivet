...
USER_ENV_PATH="$NOMAD_ALLOC_DIR/user-env.json"
echo $NOMAD_META_user_env | jq -r 'to_entries | map("\(.key)=\(.value)")' > $USER_ENV_PATH

# get new envb in here somehow
# check bounds
...
.process.env = (
  $(cat $USER_ENV_PATH | jq -c '.') + 
  $(jq -c '.process.env' $OVERRIDE_CONFIG) + 
  .process.env
) |

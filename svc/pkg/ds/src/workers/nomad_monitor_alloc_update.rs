...
async fn worker(
	...
	let alloc_state_json = serde_json::value::to_raw_value(&alloc)?.to_string();
	...
	match main_task_state {
		TaskState::Pending => {
			let run_row = sql_fetch_optional!(
				...
				SET nomad_alloc_state = $2
				WHERE nomad_dispatched_job_id = $1
				RETURNING server_id
				",
				job_id,
				&alloc_state_json,
			...
			let run_row = sql_fetch_optional!(
				...
				WITH
					...
					update_servers AS (
						...
						FROM select_server
						WHERE
							servers.server_id = select_server.server_id AND
							servers.start_ts IS NULL
						RETURNING 1
					),
					update_server_nomad AS (
						...
						FROM select_server
						WHERE server_nomad.server_id = select_server.server_id
						RETURNING 1
					)
				SELECT * FROM select_server
				",
				job_id,
				ctx.ts(),
				&alloc_state_json,
		...
		TaskState::Dead => {
			let run_row = sql_fetch_optional!(
				...
				WITH
					select_server AS (
						SELECT s.server_id, s.finish_ts
						FROM db_ds.server_nomad AS sn
						INNER JOIN db_ds.servers AS s
						ON s.server_id = sn.server_id
						WHERE nomad_dispatched_job_id = $1

...
pub async fn build_ds(
	...
	let dynamic_servers = sql_fetch_all!(
		...
		SELECT
			...
			server_ports.nomad_label AS label,
			server_ports.nomad_ip,
			server_ports.nomad_source,
		...
		ON
			server_ports.server_id = servers.server_id
		JOIN
			db_dynamic_servers.docker_ports_protocol_game_guard
				ON
					server_ports.server_id = docker_ports_protocol_game_guard.server_id
				AND
					server_ports.nomad_label = CONCAT('ds_', docker_ports_protocol_game_guard.port_name)
		WHERE
			servers.datacenter_id = $1
	",
		dc_id
	)
	.await?;
...
fn ds_register_proxied_port(
	...
	let proxy_protocol = unwrap!(backend::dynamic_servers::GameGuardProtocol::from_i32(

...
use rivet_config::config::RivetAccessKind;
...
pub async fn build_bootstrap_data(
	...
	Ok(models::CloudBootstrapResponse {
		...
		domains: if let (Some(main), Some(cdn), Some(job)) = (
			config.server()?.rivet.domain.main.clone(),
			config.server()?.rivet.domain.cdn.clone(),
			config.server()?.rivet.domain.job.clone(),
		) {
		...
		origins: Box::new(models::CloudBootstrapOrigins {
			hub: config.server()?.rivet.origin.hub.clone(),
		}),
		...
		login_methods: Box::new(models::CloudBootstrapLoginMethods {
			access_token: server_config.rivet.access_token_login,

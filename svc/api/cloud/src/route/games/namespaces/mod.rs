	...
	let cdn_ns_config = {
		...
		let hostnames_res =
			futures_util::stream::iter(custom_hostname_namespace.identifiers.iter().cloned().map(
				|identifier| {
					...
					async move {
						if let Some(game_zone_id) = &config.server()?.cloudflare.zone.game {
							let res = client
								.get(format!(
							...
							identifier = identifier,
								...
								.header(
									...
									format!(
										"Bearer {}",
										config.server()?.cloudflare.auth_token.read()
									),
								)
								.send()
								.await?;
		...
		let domains = cdn
			...
			.map(|domain| {
				...
				{
					let domain_cdn = unwrap_ref!(ctx.config().server()?.rivet.domain.cdn);

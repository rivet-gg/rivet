...
pub async fn setup_identity(
	...
	.await?
	{
		...
		let user_token_claims =
			rivet_claims::decode(&ctx.config().server()?.jwt_key.public, &game_user_res.token)??;
...
async fn attempt_setup_existing_identity_token(
	...
	let req_user_token_claims =
		match rivet_claims::decode(&ctx.config().server()?.jwt_key.public, req_user_token) {
			Ok(Ok(x)) => x,
			Ok(Err(err)) => {
		...
		> req_user_token_claims.iat + util_game_user::GAME_USER_TOKEN_REFRESH_AGE
	{
		...
		// Decode the token
		let user_token_claims =
			rivet_claims::decode(&ctx.config().server()?.jwt_key.public, &user_token.token)??;

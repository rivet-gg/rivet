...
pub struct GetContainerMetricsQuery {
	#[serde(flatten)]
...
pub async fn get_metrics(
	...
	query: GetContainerMetricsQuery,
) -> GlobalResult<models::ContainersGetContainerMetricsResponse> {
	...
	// Validate the container belongs to this game/env
	assert::actor_for_env(&ctx, &[container_id], game_id, env_id, None).await?;
	...
	if interval_seconds == 0 {
		bail_with!(CONTAINER_METRICS_INVALID_INTERVAL);
	}
	...
	Ok(models::ContainersGetContainerMetricsResponse {

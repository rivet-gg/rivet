...
async fn empty(ctx: TestCtx) {
	...
	let res = op!([ctx] mm_config_lobby_group_resolve_version {
		lobby_group_ids: vec![lobby_group.lobby_group_id.unwrap(), Uuid::new_v4().into()],
	})
	.await
	.unwrap();

	assert_eq!(1, res.versions.len());
	let version = res.versions.first().unwrap();
	assert_eq!(version_res.version_id, version.version_id);
}

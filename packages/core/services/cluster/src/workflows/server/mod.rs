...
async fn lifecycle(
	...
	match state.run(ctx).await? {
		...
		Main::NomadRegistered(sig) => {
			...
			// Registered after drain started, run drain again
			if state.draining {
				ctx.v(2)
					.workflow(drain::Input {
						datacenter_id: input.datacenter_id,
						server_id: input.server_id,
						pool_type: input.pool_type,
					})
		...
		Main::_PegboardRegistered(_) => {
			// Deprecated
		}
		Main::PegboardRegistered(sig) => {
...
async fn cleanup(
	...
	match provider {
		...
		Provider::Linode => {
			...
			ctx.workflow::<linode::workflows::server::Input>(provider_server_workflow_id)
				.output()
				.await?;
		}
	}
...
#[signal("cluster_server_pegboard_registered")]
pub struct PegboardRegistered {
	pub client_id: Uuid,
}

/// Deprecated.
#[signal("pegboard_client_registered")]
struct _PegboardRegistered {}

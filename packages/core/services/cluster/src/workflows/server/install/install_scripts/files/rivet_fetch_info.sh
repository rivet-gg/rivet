...
echo 'Fetching rivet server info'
response=$(
	curl -f \
...
datacenter_name_id=$(echo $response | jq -r '.datacenter_name_id')
...
sed -i "s/___DATACENTER_NAME_ID___/$datacenter_name_id/g" $initialize_script

...
pub async fn upsert(ctx: &OperationCtx, input: &Input) -> GlobalResult<Output> {
	...
	validate_selector_tags(&input.selector_tags)?;

	let now = ctx.ts();
	let selector_tags_json = util::serde::Raw::new(&input.selector_tags)?;
	...
	let (route_id, created) = rivet_pools::utils::crdb::tx(&ctx.crdb().await?, |tx| {
		...
		let selector_tags_json = selector_tags_json.clone();
		let now = now;
		...
		async move {
			...
			if let Some(existing_id) = route_id {
				...
				sql_execute!(
					...
					SET hostname = $1, path = $2, route_subpaths = $3, selector_tags = $4, update_ts = $5
					WHERE route_id = $6 AND namespace_id = $7 AND delete_ts IS NULL
					",
					&input_hostname,
					&input_path,
				...
				sql_execute!(
					...
					INSERT INTO db_route.routes (
						route_id, namespace_id, name_id, hostname, path, route_subpaths,
						selector_tags, create_ts, update_ts, delete_ts
					)
...
fn validate_path(path: &str) -> GlobalResult<()> {
	...
	ensure_with!(
		...
		ROUTE_INVALID_PATH,
		msg = format!("path can have at most {} components", crate::utils::MAX_PATH_COMPONENTS)
	);
...
/// Validates the selector_tags for a route
fn validate_selector_tags(selector_tags: &HashMap<String, String>) -> GlobalResult<()> {
	ensure_with!(
		selector_tags.len() <= 8,
		ROUTE_INVALID_SELECTOR_TAGS,
		msg = "selector_tags can have at most 8 entries"
	);

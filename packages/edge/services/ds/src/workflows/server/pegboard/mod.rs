					...
					match sig {
						Main::ActorStateUpdate(sig) => match sig.state {
							...
							pp::ActorState::Running { ports, .. } => {
								ctx.activity(UpdatePortsInput { ports }).await?;
...
async fn allocate_actor(
	...
	let res = ctx
		.op(edge_pegboard::ops::client::reserve::Input {
...
async fn update_ports(ctx: &ActivityCtx, input: &UpdatePortsInput) -> GlobalResult<()> {
	...
	for (label, port) in &input.ports {
		sql_execute!(
			[ctx, @tx &mut tx]
			"
			INSERT INTO server_proxied_ports (
				label,
				source,
				ip
			)
...
async fn reschedule_actor(
	...
	ctx.activity(ClearPortsInput {}).await?;
...
async fn clear_ports(ctx: &ActivityCtx, input: &ClearPortsInput) -> GlobalResult<()> {
	sql_execute!(
		[ctx]
		"
		DELETE FROM server_proxied_ports
		",
	)
	.await?;
...
join_signal!(Main {
	ActorStateUpdate(edge_pegboard::workflows::actor::StateUpdate),
	Upgrade,
	Destroy,
});

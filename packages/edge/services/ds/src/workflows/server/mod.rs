...
async fn insert_db(ctx: &ActivityCtx, input: &InsertDbInput) -> GlobalResult<i64> {
	...
	// TODO: implement AND move to its own activity
	// Select ingress ports from global idx
	// let ports = ctx.op(crate::ops::server::allocate_ingress_ports::Input {
	// 	server_id: input.server_id,
	// 	ports: ,
	// }).await?;
	// ensure_eq!(ports.len(), input.network_ports.len(), "not all ports allocated");
	let mut ingress_ports = std::iter::repeat(0i32);
		...
		match port.routing {
			Routing::GameGuard { protocol } => {
				sql_execute!(
					...
					protocol as i32,
					unwrap!(ingress_ports.next(), "missing ingress port"),
				)
				.await?;
			}

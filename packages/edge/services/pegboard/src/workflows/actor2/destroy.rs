...
#[activity(UpdateFdb)]
pub async fn update_fdb(ctx: &ActivityCtx, input: &UpdateFdbInput) -> GlobalResult<UpdateFdbOutput> {
	let pool = ctx.sqlite().await?;
...
pub(crate) async fn clear_ports_and_resources(
	...
	if let (
		...
		let old_runner_allocation_key =
			keys::datacenter::RunnersByRemainingSlotsKey::new(runner_remaining_slots, runner_id);

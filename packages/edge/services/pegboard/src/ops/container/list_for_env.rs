...
#[operation]
pub async fn pegboard_container_list_for_env(
	ctx: &OperationCtx,
	input: &Input,
) -> GlobalResult<Output> {
	let actors = ctx
		...
		.run(|tx, _mc| async move {
			let container_subspace =
				keys::subspace().subspace(&keys::env::ContainerKey::subspace(input.env_id));
			let (start2, end2) = container_subspace.range();

			let end2 = if let Some(created_before) = input.created_before {
				fdb_util::end_of_key_range(&keys::subspace().pack(
					&keys::env::ContainerKey::subspace_with_create_ts(input.env_id, created_before),
				))
			} else {
				end2
			};
			...
			while let Some(entry) = stream2.try_next().await? {
				let actor_key = keys::subspace()
					.unpack::<keys::env::ContainerKey>(entry.key())

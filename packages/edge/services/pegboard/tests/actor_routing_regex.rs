...
mod tests {
	...
	use rivet_pegboard::util::{
		build_actor_hostname_and_path, build_actor_hostname_and_path_regex,
	};
	use uuid::Uuid;
	...
	fn test_hostname_routing_regex() {
		...
		for hostname in invalid_hostnames {
			assert!(
				hostname_regex.captures(hostname).is_none(),
				"Should not match invalid hostname: {}",
				hostname
			);
		}
	}
	...
	fn test_path_routing_regex() {
		...
		for path in invalid_paths {
			assert!(
				path_regex.captures(path).is_none(),
				"Should not match invalid path: {}",
				path
			);
		}
	}

		...
		match msg? {
			Message::Binary(buf) => {
				...
				if let protocol::ToServer::Init { .. } = &packet {
					...
					let workflow_id = ctx
						.workflow(edge_pegboard::workflows::client::Input { client_id, flavor })
...
async fn update_ping_thread_inner(
	...
	loop {
		...
		for (client_id, flavor, workflow_id) in clients {
			...
			ctx.op(edge_pegboard::ops::client::update_allocation_idx::Input {
				client_id,
				client_workflow_id: workflow_id,
				flavor,
				action: edge_pegboard::ops::client::update_allocation_idx::Action::UpdatePing,
			})
			.await?;
		}
	}

...
pub async fn get(
	...
	Ok(models::ActorsV1GetActorResponse {
...
pub async fn create(
	...
	let created_fut = if network.wait_ready.unwrap_or_default() {
		...
		let mut created_sub = ctx
			.subscribe::<pegboard::workflows::actor::v1::CreateComplete>(("actor_id", actor_id))
			.await?;
	...
	ctx.workflow(pegboard::workflows::actor::v1::Input {
		actor_id,
		env_id,
		tags,
		resources,
		...
		network_ports: network
			...
			.map(|(s, p)| GlobalResult::Ok((
				s.clone(),
				pegboard::workflows::actor::v1::Port {
	...
	Ok(models::ActorsV1CreateActorResponse {
		actor: Box::new(pegboard::types::v1::convert_actor_to_api(
			actor.clone(),
			dc,
		)?),
...
pub async fn destroy(
	...
	let actor = assert::actor_for_env_v1(&ctx, actor_id, game_id, env_id, None).await?;
	...
	ctx.signal(pegboard::workflows::actor::v1::Destroy {
		override_kill_timeout_ms: query.override_kill_timeout,
	})
	.to_workflow::<pegboard::workflows::actor::v1::Workflow>()
	.tag("actor_id", actor_id)
	.send()
	.await?;
...
pub async fn upgrade(
	...
	assert::actor_for_env_v1(&ctx, actor_id, game_id, env_id, None).await?;
	...
	ctx.signal(pegboard::workflows::actor::v1::Upgrade {
		image_id: build.build_id,
	})
	.to_workflow::<pegboard::workflows::actor::v1::Workflow>()
	.tag("actor_id", actor_id)
...
pub async fn upgrade_all(
	...
	loop {
		let list_res = ctx
			.op(pegboard::ops::actor::v1::list_for_env::Input {
				env_id,
				tags: tags.clone(),
	...
	Ok(models::ActorsV1UpgradeAllActorsResponse {
...
pub async fn list_actors(
	...
	let actors = actors_res
		.actors
		.into_iter()
		.map(|a| pegboard::types::v1::convert_actor_to_api(a, &dc))

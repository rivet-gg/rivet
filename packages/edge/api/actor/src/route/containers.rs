...
pub async fn get(
	...
	let actors_res = ctx
		...
		.await?;
	let actor = unwrap_with!(actors_res.actors.first(), CONTAINER_NOT_FOUND);
	...
	Ok(models::ContainersGetContainerResponse {
		container: Box::new(pegboard::types::convert_container_to_api(
			actor.clone(),
			dc,
		)?),
	})
...
pub async fn create(
	...
	tracing::info!(?actor_id, ?tags, "creating container with tags");
	...
	ctx.workflow(pegboard::workflows::actor::Input {
		...
		env_id,
		tags,
		resources: Some((*body.resources).api_try_into()?),
		...
		args: Vec::new(),
		environment: body.environment.unwrap_or_default().as_hashable(),
		network_ports: body
			.ports
			.unwrap_or_default()
			.into_iter()
			.map(|(s, p)| {
				let port =
					pegboard::workflows::actor::convert_port_request_from_api(s.as_str(), p)?;
				Ok((s, port))
			})
	...
	tokio::select! {
		...
		res = fail_sub.next() => {
			let msg = res?;
			bail_with!(CONTAINER_FAILED_TO_CREATE, error = msg.message);
		}
		res = destroy_sub.next() => {
			res?;
			bail_with!(CONTAINER_FAILED_TO_CREATE, error = "Container failed before reaching a ready state.");
		}
	}
	...
	Ok(models::ContainersCreateContainerResponse {
		container: Box::new(pegboard::types::convert_container_to_api(
			actor.clone(),
			dc,
		)?),
...
pub async fn destroy(
	...
	let mut sub = ctx
		.subscribe::<pegboard::workflows::actor::DestroyStarted>(("actor_id", container_id))
		.await?;

	// Get actor after sub is created
	let actor = assert::container_for_env(&ctx, container_id, game_id, env_id, None).await?;
...
pub async fn upgrade(
	...
	assert::container_for_env(&ctx, container_id, game_id, env_id, None).await?;
...
pub async fn upgrade_all(
	...
	loop {
		let list_res = ctx
			.op(pegboard::ops::container::list_for_env::Input {
				env_id,
				tags: tags.clone(),
	...
	Ok(models::ContainersUpgradeAllContainersResponse {
...
pub async fn list_containers(
	...
	let actors = actors_res
		.actors
		.into_iter()
		.map(|a| pegboard::types::convert_container_to_api(a, &dc))

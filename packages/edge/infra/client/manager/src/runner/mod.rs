...
/// How many times to retry getting the PID before timing out.
const SOCKET_READY_RETRIES: usize = 15;
/// How often to retry getting the PID before timing out.
const SOCKET_READY_INTERVAL: Duration = Duration::from_millis(125);
// TODO: Replace with channel
/// How many times to retry getting the PID before timing out.
const GET_PID_RETRIES: usize = 16;
/// How often to retry getting the PID before timing out.
const GET_PID_INTERVAL: Duration = Duration::from_millis(50);
...
pub struct Runner {
	runner_id: Uuid,
	comms: Comms,
	/// Config of the first actor allocated to this runner.
	actor_config: protocol::ActorConfig,
	pid: RwLock<Option<Pid>>,

	actor_observer_tx: broadcast::Sender<(Uuid, runner_protocol::ActorState)>,
}

impl Runner {
	pub fn new(runner_id: Uuid, comms: Comms, actor_config: protocol::ActorConfig) -> Self {
		Runner {
			runner_id,
			comms,
			actor_config,
	...
	async fn receive_messages(
		...
		loop {
			match tokio::time::timeout(PING_TIMEOUT, ws_rx.next()).await {
				...
				Err(_) => bail!("socket timed out, killing runner"),
			}
		}
	}
	...
	async fn process_packet(&self, packet: runner_protocol::ToManager) -> Result<()> {
		...
		match packet {
			...
			runner_protocol::ToManager::ActorStateUpdate { actor_id, state } => {
				// NOTE: No receivers is not an error
				let _ = self.actor_observer_tx.send((actor_id, state));
			}
		}
		...
		match &self.comms {
			...
			Comms::Socket(socket) => {
				...
				let mut guard = loop {
					...
					if attempts == 0 {
						tracing::warn!(runner_id=?self.runner_id, "socket not yet attached, can't send message. retrying");
					}

					attempts += 1;
					if attempts > SOCKET_READY_RETRIES {
						bail!("timed out waiting for runner socket to attach");
					}
	...
	pub async fn start(
		...
		// Write runner to DB
		let actor_config_json = serde_json::to_vec(&self.actor_config)?;
		...
		let proxied_ports = match self.setup(&ctx, actor_id, generation).await {
	...
	async fn run(&self, ctx: &Ctx) -> Result<()> {
		let mut runner_env = vec![(
			"ROOT_USER_ENABLED",
			self.actor_config.root_user_enabled.to_string(),
		)];
		// TODO: Logs
	...
	pub fn new_actor_observer(&self, actor_id: Uuid) -> ActorObserver {
		ActorObserver::new(actor_id, self.actor_observer_tx.subscribe())
	}
...
impl Runner {
	pub async fn pid(&self) -> Result<Pid> {
		...
		loop {
			...
			tokio::time::sleep(GET_PID_INTERVAL).await;
		}
	}
...
pub struct ActorObserver {
	actor_id: Uuid,
	sub: broadcast::Receiver<(Uuid, runner_protocol::ActorState)>,
}

impl ActorObserver {
	fn new(actor_id: Uuid, sub: broadcast::Receiver<(Uuid, runner_protocol::ActorState)>) -> Self {
		ActorObserver { actor_id, sub }
	}
	pub async fn next(&mut self) -> Option<runner_protocol::ActorState> {
		loop {
			let Ok((other_actor_id, state)) = self.sub.recv().await else {
				tracing::error!("actor observer channel dropped");

				return None;
			};

			if self.actor_id == other_actor_id {
				return Some(state);
			}
		}
	}

...
impl Runner {
	pub async fn setup(
		...
		let (_, ports) = tokio::try_join!(
			...
			async {
				let ports = self.bind_ports(ctx, actor_id, generation).await?;

				if let protocol::NetworkMode::Bridge = self.actor_config.network_mode {
	...
	pub async fn make_fs(&self, ctx: &Ctx) -> Result<()> {
		...
		if ctx.config().runner.use_mounts() {
			...
			fs_img
				.set_len(self.actor_config.resources.disk as u64 * 1024 * 1024)
				.await?;
		...
		tracing::info!(runner_id=?self.runner_id, duration_seconds=duration, "fs creation completed");
	...
	pub async fn download_image(&self, ctx: &Ctx) -> Result<()> {
		let timer = std::time::Instant::now();
		tracing::info!(runner_id=?self.runner_id, "downloading artifact");
		...
		let addresses = crate::utils::get_image_addresses(
			ctx,
			self.actor_config.image.id,
			&self.actor_config.image.artifact_url_stub,
			self.actor_config.image.fallback_artifact_url.as_deref(),
		)
		.await?;
		...
		tracing::info!(
			runner_id=?self.runner_id,
			image_id=?self.actor_config.image.id,
			addresses=?addresses,
		...
		for url in &addresses {
			tracing::info!(runner_id=?self.runner_id, ?url, "attempting download");
			...
			let shell_cmd = match (
				self.actor_config.image.kind,
				self.actor_config.image.compression,
			) {
			...
			match cmd_result {
				Ok(output) if output.status.success() => {
					...
					tracing::info!(runner_id=?self.runner_id, duration_seconds=duration, "artifact download completed");
	...
	pub async fn setup_oci_bundle(
		...
		if let protocol::ImageKind::DockerImage = self.actor_config.image.kind {
			let docker_image_path = fs_path.join("docker-image.tar");
		...
		let config = oci_config::config(oci_config::ConfigOpts {
			...
			cpu: self.actor_config.resources.cpu,
			memory: self.actor_config.resources.memory,
			memory_max: self.actor_config.resources.memory_max,
		})?;
	...
	pub async fn cleanup_setup(&self, ctx: &Ctx) {
		...
		match self.actor_config.image.kind {
	...
	fn netns_path(&self) -> PathBuf {
		if let protocol::NetworkMode::Host = self.actor_config.network_mode {
			// Host network

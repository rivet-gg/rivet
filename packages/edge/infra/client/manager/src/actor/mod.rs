...
impl Actor {
	...
	async fn run(self: &Arc<Self>, ctx: &Arc<Ctx>) -> Result<Option<runner::ActorObserver>> {
		...
		// condition
		let actor_observer = match self.config.image.allocation_type {
			protocol::ImageAllocationType::Single => None,
			protocol::ImageAllocationType::Multi => {
				Some(self.runner.new_actor_observer(self.actor_id))
			}
		};
			...
			.context("should have runner config")?
		{
			protocol::ActorRunner::New { .. } => {
				let ports = self
					.runner
					.start(ctx, self.actor_id, self.generation)
					.await?;
			...
			protocol::ActorRunner::Existing { .. } => {
				// TODO: add ports to the existing runner somehow

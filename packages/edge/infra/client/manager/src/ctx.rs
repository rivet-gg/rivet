...
impl Ctx {
		...
		match command.inner.deserialize()? {
			protocol::Command::StartActor {
				...
				if actors.contains_key(&(actor_id, generation)) {
					...
					if let Some(runner) = self.get_or_create_runner(&config).await? {
						...
						tracing::error!(?actor_id, ?generation, runner=?config.runner, "timed out waiting for actor's runner to be inserted, considering actor lost");
	...
	async fn get_or_create_runner(
		...
		{
			protocol::ActorRunner::New { runner_id } => {
				let mut runners = self.runners.write().await;

				let comms = match &actor_config.image.allocation_type {
					protocol::ImageAllocationType::Single => runner::Comms::Basic,
					protocol::ImageAllocationType::Multi => runner::Comms::socket(),
				};

				let runner = Arc::new(Runner::new(*runner_id, comms, actor_config.clone()));
...
impl Ctx {
		...
		let ((last_workflow_id,), runner_rows) = tokio::try_join!(
			...
			utils::sql::query(|| async {
				sqlx::query_as::<_, RunnerRow>(indoc!(
					"
					SELECT runner_id, comms, actor_config, pid
					FROM runners
	...
	async fn rebuild(self: &Arc<Self>, workflow_id: Uuid) -> Result<()> {
		...
		for row in runner_rows {
			...
			let actor_config = serde_json::from_slice::<protocol::ActorConfig>(&row.actor_config)?;
		...
		for row in actor_rows {
			...
			// updates here
			let actor_observer =
				if let protocol::ImageAllocationType::Multi = config.image.allocation_type {
					Some(runner.new_actor_observer(row.actor_id))
				} else {
					None
				};

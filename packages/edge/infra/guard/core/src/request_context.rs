...
lazy_static! {
	static ref GUARD_IDS: (Option<Uuid>, Option<Uuid>, Option<Uuid>) = {
		let datacenter_id = std::env::var("RIVET_DATACENTER_ID")
			.ok()
			.and_then(|s| s.parse::<Uuid>().ok());
		let cluster_id = std::env::var("RIVET_CLUSTER_ID")
			.ok()
			.and_then(|s| s.parse::<Uuid>().ok());
		let server_id = std::env::var("RIVET_SERVER_ID")
			.ok()
			.and_then(|s| s.parse::<Uuid>().ok());
		(datacenter_id, cluster_id, server_id)
	};
}
...
impl RequestContext {
	...
	pub fn new_with_request_id(
		...
		let (datacenter_id, cluster_id, server_id) = *GUARD_IDS;
		...
		Self {
			...
			guard_datacenter_id: datacenter_id,
			guard_cluster_id: cluster_id,
			guard_server_id: server_id,
	...
	pub async fn insert_event(&mut self) -> GlobalResult<()> {

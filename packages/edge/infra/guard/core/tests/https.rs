...
use rivet_guard_core::{
	proxy_service::{
		MiddlewareResponse, PortType, RouteTarget, RoutingResponse, RoutingResult, RoutingTimeout,
	},
	CertResolverFn,
};
...
fn load_certified_key(
	...
	let key_der = private_key(key_reader)?.ok_or("No private key found")?;

	let signing_key =
		any_supported_type(&key_der).map_err(|e| format!("Failed to load private key: {}", e))?;
...
async fn start_guard_with_https(
	...
	routing_fn: Arc<
		dyn for<'a> Fn(&'a str, &'a str, PortType) -> BoxFuture<'a, GlobalResult<RoutingResponse>>
			+ Send
			+ Sync,
	>,
	middleware_fn: Arc<
		dyn for<'a> Fn(&'a Uuid) -> BoxFuture<'a, GlobalResult<MiddlewareResponse>> + Send + Sync,
	>,
	...
	tokio::spawn(async move {
		let server_task = tokio::spawn(async move {
			let result =
				rivet_guard_core::run_server(config, routing_fn, middleware_fn, cert_resolver_fn)
					.await;
...
async fn test_https_with_tls() {
	...
	let fixtures_abs_path = std::path::PathBuf::from(
		"/Users/nathan/rivet/ee/oss/packages/edge/infra/guard/core/tests/fixtures/tls",
	);
	assert!(

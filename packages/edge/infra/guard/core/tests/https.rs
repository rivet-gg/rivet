...
use common::{
    ...
    init_tracing, TestServer,
};
use rivet_guard_core::{
    proxy_service::{
        PortType, RouteTarget, RoutingResult, RoutingResponse, RoutingTimeout,
        MiddlewareResponse
    },
    CertResolverFn
};
...
fn load_certified_key(cert_path: &std::path::Path, key_path: &std::path::Path) -> Result<CertifiedKey, Box<dyn std::error::Error>> {
    ...
    let key_der = private_key(key_reader)?
...
async fn start_guard_with_https(
    ...
    routing_fn: Arc<dyn for<'a> Fn(&'a str, &'a str, PortType) -> BoxFuture<'a, GlobalResult<RoutingResponse>> + Send + Sync>,
    middleware_fn: Arc<dyn for<'a> Fn(&'a Uuid) -> BoxFuture<'a, GlobalResult<MiddlewareResponse>> + Send + Sync>,
    ...
    let new_config = create_test_config(|guard_config| {
        ...
        if let Ok(original_guard) = config.guard() {
...
async fn test_https_with_tls() {
    ...
    let test_server = TestServer::with_handler(|req, _request_log| {
        ...
        Box::pin(async move { Ok::<_, std::convert::Infallible>(response) })
    }).await;
    let test_server_addr = test_server.addr;
    
    // Set up paths to TLS certificates
    let fixtures_abs_path = std::path::PathBuf::from("/Users/nathan/rivet/ee/oss/packages/edge/infra/guard/core/tests/fixtures/tls");
    assert!(fixtures_abs_path.exists(), "Fixtures directory doesn't exist");
    ...
    let routing_fn: rivet_guard_core::proxy_service::RoutingFn = Arc::new(move |hostname: &str, _path: &str, _port_type: PortType| {
        ...
        Box::pin(async move {
            ...
            let route_target = RouteTarget {
                ...
                server_id: Some(Uuid::parse_str("33333333-3333-3333-3333-333333333333").unwrap()),
    ...
    let config = create_test_config(|guard_config| {
        ...
        guard_config.https = Some(rivet_config::config::guard::Https {
            ...
            tls: rivet_config::config::guard::Tls {
                cert_dir: fixtures_abs_path.clone(),
                api_cert_path: api_cert_path.clone(),
                api_key_path: api_key_path.clone(),
                job_cert_path: job_cert_path.clone(),
                job_key_path: job_key_path.clone(),
            },
        });
    });
    
    // Start the guard server with HTTPS and TLS support
    let (addrs, _shutdown) = start_guard_with_https(
        config,
        routing_fn,
        middleware_fn,
        Some(cert_resolver_fn),
    ).await;
    ...
    assert!(http_addr.parse::<SocketAddr>().is_ok(), "Invalid HTTP address: {}", http_addr);
    assert!(https_addr.parse::<SocketAddr>().is_ok(), "Invalid HTTPS address: {}", https_addr);
    ...
    let tls_status = tokio::process::Command::new("sh")
        .arg("-c")
        .arg(format!("echo Q | openssl s_client -connect 127.0.0.1:{} -servername {} -brief", https_port, test_host))
        .status()
        .await
        .expect("Failed to execute openssl command");

    assert!(tls_status.success(), "Expected openssl command to exit with code 0, got: {:?}", tls_status);
    ...
    let status = tokio::process::Command::new("curl")
        .args([
            ...
            "--max-time", "5",
            "--connect-timeout", "5",
            "--resolve", &format!("{}:{}:127.0.0.1", test_host, https_port),
    ...
    assert!(status.success(), "Expected curl command to exit with code 0, got: {:?}", status);
    
    // Verify the request reached the test server
    assert!(test_server.request_count() >= 1, "Expected at least the HTTP request to reach the server");
    
    let last_request = test_server.last_request().unwrap();
    assert_eq!(last_request.method, "GET", "Expected GET method for HTTPS request");
    assert_eq!(last_request.uri, "/test-http", "Unexpected URI path for HTTPS request");
    ...
    let gameinc_status = tokio::process::Command::new("curl")
        .args([
            ...
            "--max-time", "5",
            "--connect-timeout", "5",
            "--resolve", &format!("{}:{}:127.0.0.1", gameinc_host, https_port),
    ...
    assert!(gameinc_status.success(), "curl command failed for gameinc.io: {:?}", gameinc_status);
    assert_eq!(gameinc_status.code(), Some(0), "Expected exit code 0 for gameinc.io curl command");
    ...
    let invalid_status = tokio::process::Command::new("curl")
        .args([
            ...
            "--max-time", "5",
            "--connect-timeout", "5",
            "--resolve", &format!("{}:{}:127.0.0.1", invalid_host, https_port),
    ...
    assert!(!invalid_status.success(), "Expected curl to fail with invalid domain");
    assert_ne!(invalid_status.code(), Some(0), "Expected non-zero exit code for invalid domain curl command");
}

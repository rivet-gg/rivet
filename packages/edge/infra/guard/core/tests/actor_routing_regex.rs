...
use rivet_guard_core::util::{build_actor_hostname_and_path_regex, build_actor_hostname_and_path};
...
fn test_build_actor_hostname_and_path_regex() {
    ...
    let (hostname_regex, _) = build_actor_hostname_and_path_regex(
    ...
    let captures = hostname_regex.captures("123e4567-e89b-12d3-a456-426614174000-http.example.com").unwrap();
    assert_eq!(captures.name("actor_id").unwrap().as_str(), "123e4567-e89b-12d3-a456-426614174000");
    assert_eq!(captures.name("port_name").unwrap().as_str(), "http");
    
    // Test path-based routing
    let (_, path_regex) = build_actor_hostname_and_path_regex(
        EndpointType::Path,
        "example.com",
    ).unwrap().unwrap();
    ...
    let captures = path_regex.captures("/123e4567-e89b-12d3-a456-426614174000-http/some/path").unwrap();
    assert_eq!(captures.name("actor_id").unwrap().as_str(), "123e4567-e89b-12d3-a456-426614174000");
    assert_eq!(captures.name("port_name").unwrap().as_str(), "http");
}
...
fn test_build_actor_hostname_and_path() {
    ...
    assert_eq!(hostname, "123e4567-e89b-12d3-a456-426614174000-http.example.com");
...
fn test_invalid_inputs() {
    ...
    let hostname_regex = build_actor_hostname_and_path_regex(
        EndpointType::Hostname,
        "example.com",
    ).unwrap().unwrap().0;

...
async fn run_client() {
    let (ws_stream, _) = tokio_tungstenite::connect_async("ws://localhost:3000").await.expect("Failed to connect");
    println!("WebSocket handshake has been successfully completed");

    let (mut write, mut read) = ws_stream.split();

	write.send(tungstenite::protocol::Message::Text("Test".into())).await.unwrap();

...
use tokio_tungstenite::{
    connect_async,
    tungstenite::protocol::Message as TokioMessage,
};
...
async fn test_websocket_echo() {
    ...
    println!("WebSocket connection established with status: {}", response.status());
    ...
    ws_stream.send(TokioMessage::Text(test_message.into()))
        .await
    ...
    ws_stream.send(TokioMessage::Binary(binary_data.clone().into()))
        .await
    ...
    ws_stream.send(TokioMessage::Ping(ping_data.clone().into()))
        .await
            ...
            tokio::spawn(async move {
                let service = service_fn(|req: Request<Incoming>| {
                    async {
                        ...
                        println!("Server: Returning response with status: {}", response.status());
                        for (name, value) in response.headers() {
                            if let Ok(val) = value.to_str() {
                                println!("Server:   {} = {}", name, val);
                            }
                        ...
                        tokio::spawn(async move {
                            ...
                            match websocket.await {
                                Ok(ws_stream) => {
                                    println!("Server: WebSocket connection established successfully");
                                            ...
                                            Ok(msg) => {
                                                match &msg {
                                                    ...
                                                    HyperMessage::Ping(data) => {
                                                        println!("Server: Received ping of {} bytes", data.len());
                                                    },
                                                    HyperMessage::Pong(_) => {
                                                ...
                                                let response = match msg {
                                                    HyperMessage::Ping(data) => {
                                                        println!("Server: Converting ping to pong response");
                                                ...
                                                if let Err(e) = write.send(response).await {
                                                    println!("Server: Error sending message: {}", e);
                                                    break;
                                                }

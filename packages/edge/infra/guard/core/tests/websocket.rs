...
use common::{
	create_test_config, create_test_middleware_fn, init_tracing, start_guard_with_middleware,
	TestServer,
};
			...
			Box::pin(async move {
				...
				let (response, websocket) = match hyper_tungstenite::upgrade(req, None) {
					...
					Err(e) => {
						...
						tracing::error!(
							"Echo server: Failed to upgrade WebSocket connection: {}",
							e
						);
						return Ok(hyper::Response::builder()
							...
							while let Some(msg_result) = ws_receiver.next().await {
								match msg_result {
									Ok(msg) => {
										match &msg {
											...
											HyperMessage::Close(_) => {
												...
												tracing::info!(
													"Echo server: Received close message"
												);
											}
											_ => {
												println!(
										...
										match msg {
											HyperMessage::Text(text) => {
												...
												tracing::info!(
													"Echo server: Sending text response"
												);
												if let Err(e) =
											...
											HyperMessage::Binary(data) => {
												...
												tracing::info!(
													"Echo server: Sending binary response"
												);
												if let Err(e) =
											...
											HyperMessage::Ping(data) => {
												...
												tracing::info!(
													"Echo server: Sending pong response"
												);
												if let Err(e) =
									...
									Err(e) => {
										...
										tracing::error!(
											"Echo server: Error receiving message: {}",
											e
										);
										break;
									}

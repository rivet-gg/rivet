...
const DEFAULT_GAME_NAME: &str = "Default";
const DEFAULT_GAME_NAME_ID: &str = "default";

const DEFAULT_ENVIRONMENT_NAME: &str = "Default";
const DEFAULT_ENVIRONMENT_NAME_ID: &str = "default";
...
pub async fn start(config: rivet_config::Config, pools: rivet_pools::Pools) -> GlobalResult<()> {
	...
	let user_resolve_res = ctx
		.op(::user::ops::resolve_display_name::Input {
			display_name: DEFAULT_USER_NAME.into(),
		})
		.await?;
	...
	let team_resolve_res = op!([ctx] team_resolve_display_name {
		display_names: vec![DEFAULT_TEAM_NAME.into()],
	})
	.await?;
	...
	let game_resolve_res = op!([ctx] game_resolve_name_id {
		name_ids: vec![DEFAULT_GAME_NAME_ID.into()],
	})
	.await?;
	let game_id = if let Some(existing) = game_resolve_res.games.first() {
		...
		let create_game_res = op!([ctx] game_create {
			name_id: DEFAULT_GAME_NAME_ID.into(),
			display_name: DEFAULT_GAME_NAME.into(),
	...
	let ns_resolve_res = op!([ctx] game_namespace_resolve_name_id {
		...
		name_ids: vec![DEFAULT_ENVIRONMENT_NAME_ID.into()],
	})
	.await?;
	if ns_resolve_res.namespaces.is_empty() {
		...
		let create_ns_res = op!([ctx] game_namespace_create {
			game_id: Some(game_id.into()),
			display_name: DEFAULT_ENVIRONMENT_NAME.into(),
			version_id: Some(version_id.into()),
			name_id: DEFAULT_ENVIRONMENT_NAME_ID.into(),
		})
		.await?;

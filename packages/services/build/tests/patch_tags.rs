...
#[workflow_test]
async fn patch_tags_exclusive_game_removes_tag_on_other_build(ctx: TestCtx) {
	let game_res = op!([ctx] faker_game {
		..Default::default()
	})
	.await
	...
	ctx.op(build::ops::patch_tags::Input {
		build_id: build_a_id,
		tags: HashMap::from([
			("tag1".to_string(), Some("same".to_string())),
	...
	ctx.op(build::ops::patch_tags::Input {
		build_id: build_b_id,
		tags: HashMap::from([("tag1".to_string(), Some("same".to_string()))]),
	...
	assert_eq!(
		build_b.tags,
		HashMap::from([("tag1".to_string(), Some("same".to_string()))])
	);
}

#[workflow_test]
async fn patch_tags_exclusive_env_removes_tag_on_other_build(ctx: TestCtx) {
	let game_res = op!([ctx] faker_game {
		..Default::default()
	})
	.await
	...
	let build_a_res = op!([ctx] faker_build {
		env_id: game_res.namespace_ids.first().cloned(),
		image: backend::faker::Image::MmLobbyAutoReady as i32,
	})
	.await
	.unwrap();
...
#[workflow_test]
async fn patch_tags_exclusive_label_but_different_value(ctx: TestCtx) {
	let game_res = op!([ctx] faker_game {
		..Default::default()
	})
	.await
...
#[workflow_test]
async fn multiple_exclusive_tags(ctx: TestCtx) {
	let game_res = op!([ctx] faker_game {
		..Default::default()
	})
	.await
	...
	assert_eq!(
		build_a.tags,
		HashMap::from([("name".to_string(), Some("foo".to_string())),])
	);

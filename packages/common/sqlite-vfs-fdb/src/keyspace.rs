...
impl FdbKeySpace {
	pub fn new(prefix: &[u8]) -> Self {
		log::info!("Creating FdbKeySpace with prefix length: {}", prefix.len());
		if prefix.is_empty() {
			log::error!("ERROR: Empty prefix provided to FdbKeySpace::new!");
		} else {
			log::info!("Prefix data: {:?}", prefix);
		}
	...
	pub fn metadata_key(&self, path: &str) -> Bytes {
		...
		log::info!("Creating metadata key for path: {}", path);
		let prefix_len = self.prefix.len();
		let prefix_empty = self.prefix.is_empty();
		log::info!("Prefix check - length: {}, is_empty: {}", prefix_len, prefix_empty);
		
		if !prefix_empty && prefix_len > 0 {
			log::info!("Prefix data: {:?}", &self.prefix[..]);
		} else {
			log::error!("Warning: Empty or invalid prefix detected!");
		}

...
/// Implementation of SQLite vfs_open callback
...
/// Implementation of SQLite vfs_delete callback
pub unsafe extern "C" fn vfs_delete(
	vfs_ptr: *mut sqlite3_vfs,
	path: *const c_char,
	_sync_dir: c_int,
) -> c_int {
...
/// Implementation of SQLite vfs_access callback
pub unsafe extern "C" fn vfs_access(
	vfs_ptr: *mut sqlite3_vfs,
	path: *const c_char,
	flags: c_int,
	res_out: *mut c_int,
...
/// Implementation of SQLite vfs_fullpathname callback
pub unsafe extern "C" fn vfs_fullpathname(
	...
	let out_buf = std::slice::from_raw_parts_mut(out as *mut u8, n_out as usize);
	let copy_len = std::cmp::min(path_str.len(), n_out as usize - 1);
...
/// Implementation of SQLite vfs_current_time_int64 callback
pub unsafe extern "C" fn vfs_current_time_int64(_vfs: *mut sqlite3_vfs, time_out: *mut i64) -> c_int {
	if !time_out.is_null() {
		...
		let time = match std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH) {
			Ok(duration) => {
				// Convert to Julian Day Number times milliseconds per day
				let unix_millis = duration.as_millis() as i64;
				let julian_day_milliseconds = (crate::utils::JULIAN_DAY_UNIX_EPOCH * crate::utils::MILLISECONDS_PER_DAY) as i64 
					+ unix_millis;
				julian_day_milliseconds
			},
			Err(_) => {
...
/// Implementation of SQLite vfs_current_time callback
pub unsafe extern "C" fn vfs_current_time(vfs: *mut sqlite3_vfs, time_out: *mut f64) -> c_int {
	if time_out.is_null() {
		return SQLITE_IOERR;
	}
	...
	if result == SQLITE_OK && !time_out.is_null() {
		...
		*time_out = time_ms as f64 / crate::utils::MILLISECONDS_PER_DAY;
		return SQLITE_OK;
	}
	result
}
...
pub fn register_vfs(db: Arc<Database>) -> Result<(), FdbVfsError> {
	...
	// No need to redefine these functions - we'll use the module-level implementations
	...
	let vfs_ptr = unsafe {
		...
		(*vfs_ptr) = sqlite3_vfs {
			...
			// Use module-level implementations to avoid duplication
			xOpen: Some(self::vfs_open),
			xDelete: Some(self::vfs_delete),
			xAccess: Some(self::vfs_access),
			xFullPathname: Some(self::vfs_fullpathname),
			xDlOpen: None,
			xDlError: None,
			xDlSym: None,
			xDlClose: None,

...
unsafe fn get_file_extension<'a>(file: *mut FdbFile, context: &str) -> Result<&'a FdbFileExt, c_int> {
...
#[allow(dead_code)]
unsafe fn get_file_extension_mut<'a>(file: *mut FdbFile, context: &str) -> Result<&'a mut FdbFileExt, c_int> {
    let fdb_file = get_file_ptr(file, context)?;
...
/// Get VFS reference safely from file extension
unsafe fn get_vfs_from_ext<'a>(ext: &'a FdbFileExt, context: &str) -> Result<&'a FdbVfs, c_int> {
    let vfs_ptr = ext.vfs;
    if vfs_ptr.is_null() {
        tracing::error!("Null VFS pointer in {}", context);
        metrics::record_vfs_error("null_vfs_pointer");
        return Err(SQLITE_IOERR);
    }
    
    // Safety: We've checked the pointer is not null before dereferencing
    Ok(&*vfs_ptr)
}
...
pub unsafe extern "C" fn fdb_file_close(file: *mut sqlite3_file) -> c_int {
	...
	let path = ext_assumed.path.clone();

	// Record metrics for file close
	metrics::record_file_close(&path, true);
	tracing::debug!("Successfully closed file: {}", path);

	SQLITE_OK
}

/// Read pages from FoundationDB for a file
fn read_pages_from_fdb(
    ...
    let result = run_fdb_tx(&db, move |tx| {
        ...
        let mut data_buffer = data_buffer.clone(); // Clone and make mutable

        async move {
            let mut bytes_read = 0;
            let mut _pages_read = 0;
            let mut _partial_pages = 0;
    ...
    // Convert result type to expected return type
    match result {
        Ok((bytes_read, buffer)) => Ok((buffer, bytes_read)),
        Err(e) => Err(FdbVfsError::Other(format!("FDB error: {}", e))),
    }
}
...
pub unsafe extern "C" fn fdb_file_read(
    ...
    let read_result = read_pages_from_fdb(vfs, file_id, offset, count as usize, page_size, file_size);
...
pub unsafe extern "C" fn fdb_file_write(
	...
	let vfs = match get_vfs_from_ext(ext, "fdb_file_write") {
		Ok(vfs) => vfs,
		Err(code) => return code,
	};
	...
	let write_result = run_fdb_tx(&db, move |tx| {
		// Only clone complex types that need to be moved into the async block
		// No need to clone primitives as they implement Copy
...
pub unsafe extern "C" fn fdb_file_lock(file: *mut sqlite3_file, lock_type: c_int) -> c_int {
	...
	let ext = match get_file_extension(file as *mut FdbFile, "fdb_file_lock") {
		Ok(ext) => ext,
		Err(code) => return code,
	};
...
pub unsafe extern "C" fn fdb_file_unlock(file: *mut sqlite3_file, lock_type: c_int) -> c_int {
	...
	let ext = match get_file_extension(file as *mut FdbFile, "fdb_file_unlock") {
		Ok(ext) => ext,
		Err(code) => return code,
	};
...
pub unsafe extern "C" fn fdb_file_check_reserved_lock(
	...
	// We only need to verify the file is valid, we don't use the ext value 
	let _ext = match get_file_extension(file as *mut FdbFile, "fdb_file_check_reserved_lock") {
		Ok(ext) => ext,
		Err(code) => return code,
	};

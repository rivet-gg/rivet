...
impl FdbVfs {
	...
	pub fn get_metadata(&self, path: &str) -> FdbResult<Option<FdbFileMetadata>> {
		...
		tracing::info!("Keyspace prefix: length={}, empty={}", 
		...
		run_fdb_tx(&self.db, move |tx| {
			...
			tracing::info!("In transaction: Using metadata_key with length: {}", metadata_key_clone.len());
	...
	pub fn file_exists(&self, path: &str) -> FdbResult<bool> {
		...
		tracing::info!("File exists check result for {}: {}", path, metadata_result.is_some());

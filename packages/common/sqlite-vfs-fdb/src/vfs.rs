...
use crate::file::{FdbFile, FdbFileExt, FDB_IO_METHODS};
...
impl FdbVfs {
	...
	pub fn with_db(db: Arc<Database>) -> Result<Self, FdbVfsError> {
		...
		info!("Creating FdbVfs with prefix length: {}", prefix.len());
		info!("Prefix bytes: {:?}", prefix);
	...
	pub fn register(&self) -> Result<(), FdbVfsError> {
		debug!("Registering FdbVfs with name: {}", self.name());
	...
	pub fn get_metadata(&self, path: &str) -> FdbResult<Option<FdbFileMetadata>> {
		...
		info!("Keyspace prefix: length={}, empty={}", 
			self.keyspace.prefix_len(), 
			self.keyspace.prefix_is_empty());
		
		let metadata_key = self.keyspace.metadata_key(path);
		info!("Generated metadata_key length: {}", metadata_key.len());
		...
		run_fdb_tx(&self.db, move |tx| {
			...
			info!("In transaction: Using metadata_key with length: {}", metadata_key_clone.len());

			async move {
				info!("In async block for path: {}", path_clone);
				let result = tx.get(&metadata_key_clone, false).await?;
				info!("Got result for path {}: {:?}", path_clone, result.is_some());

				if let Some(bytes) = result {
					match FdbFileMetadata::from_bytes(&bytes) {
						Ok(metadata) => Ok(Some(metadata)),
						Err(e) => {
							warn!("Failed to parse metadata for {}: {}", path_clone, e);
							Ok(None)
						}
					}
				} else {
	...
	pub fn file_exists(&self, path: &str) -> FdbResult<bool> {
		...
		info!("File exists check result for {}: {}", path, metadata_result.is_some());
...
unsafe extern "C" fn vfs_open(
	...
	if !file_exists && !can_create {
		debug!("File does not exist and SQLITE_OPEN_CREATE not specified");
		return SQLITE_CANTOPEN;
	}
	...
	let metadata = if file_exists {
		match (*vfs_instance).get_metadata(path_str) {
			...
			Ok(None) => {
				error!("File exists but metadata not found");
				return SQLITE_CANTOPEN;
			}
			Err(e) => {
		...
		match (*vfs_instance).store_metadata(path_str, &new_metadata) {
			Ok(_) => new_metadata,
			Err(e) => {
				error!("Error creating file metadata: {}", e);
				return SQLITE_CANTOPEN;
			}
		}
	};
	...
	let fdb_file = match (file as *mut FdbFile).as_mut() {
		Some(f) => f,
		None => {
			error!("Null file pointer in vfs_open");
			return SQLITE_CANTOPEN;
		}
	};
	...
	debug!("Successfully opened file: {}", path_str);
	SQLITE_OK
}
...
unsafe extern "C" fn vfs_delete(
	...
	let path_str = CStr::from_ptr(path).to_str().unwrap_or("Invalid UTF-8");
	debug!("FDB VFS: delete called for path: {}", path_str);

	// Get the FDB VFS instance from the pAppData field
	let vfs_instance = (*vfs_ptr).pAppData as *const FdbVfs;
	if vfs_instance.is_null() {
		error!("VFS instance is null in vfs_delete");
		return SQLITE_IOERR;
	}
...
unsafe extern "C" fn vfs_access(
	...
	let vfs_instance = (*vfs_ptr).pAppData as *const FdbVfs;
	if vfs_instance.is_null() {
		error!("VFS instance is null in vfs_access");
		return SQLITE_IOERR;
	}
...
unsafe extern "C" fn vfs_fullpathname(
	...
	let path_str = CStr::from_ptr(path).to_str().unwrap_or("Invalid UTF-8");
	info!("FDB VFS: fullpathname called for path: {}", path_str);
...
pub fn register_vfs(db: Arc<Database>) -> Result<(), FdbVfsError> {
	info!("Creating and registering a SQLite VFS");
	...
	unsafe extern "C" fn vfs_current_time_int64(
		...
		if !time_out.is_null() {
			...
			if let Ok(time) = std::time::SystemTime::now()
				.duration_since(std::time::UNIX_EPOCH) {
	...
	unsafe {
		...
		if check_vfs.is_null() {
			error!("Failed to find registered VFS after registration");
			return Err(FdbVfsError::Other(
		...
		info!("Checking registered VFSs after registration:");
	...
	info!("Successfully registered FdbVfs with SQLite");
	Ok(())
}

...
/// struct for typed errors of method [`containers_metrics_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ContainersMetricsGetError {
...
pub async fn containers_metrics_get(configuration: &configuration::Configuration, container: &str, start: i32, end: i32, interval: i32, project: Option<&str>, environment: Option<&str>) -> Result<crate::models::ContainersGetContainerMetricsResponse, Error<ContainersMetricsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/containers/{container}/metrics/history", local_var_configuration.base_path, container=crate::apis::urlencode(container));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ContainersMetricsGetError> = serde_json::from_str(&local_var_content).ok();

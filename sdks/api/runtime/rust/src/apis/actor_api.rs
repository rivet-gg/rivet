...
/// Upgrades a dynamic actor.
pub async fn actor_upgrade_all(configuration: &configuration::Configuration, actor_upgrade_all_actors_request: crate::models::ActorUpgradeAllActorsRequest, project: Option<&str>, environment: Option<&str>) -> Result<crate::models::ActorUpgradeAllActorsResponse, Error<ActorUpgradeAllError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ActorUpgradeAllError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

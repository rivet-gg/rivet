...
/// struct for typed errors of method [`containers_logs_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ContainersLogsGetError {
...
pub async fn containers_logs_get(configuration: &configuration::Configuration, stream: crate::models::ContainersQueryLogStream, container_ids_json: &str, project: Option<&str>, environment: Option<&str>, search_text: Option<&str>, search_case_sensitive: Option<bool>, search_enable_regex: Option<bool>, watch_index: Option<&str>) -> Result<crate::models::ContainersGetContainerLogsResponse, Error<ContainersLogsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/containers/logs", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.query(&[("container_ids_json", &container_ids_json.to_string())]);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ContainersLogsGetError> = serde_json::from_str(&local_var_content).ok();

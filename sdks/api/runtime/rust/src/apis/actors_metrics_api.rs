...
#[serde(untagged)]
pub enum ActorsMetricsGetError {
...
pub async fn actors_metrics_get(configuration: &configuration::Configuration, start: i32, end: i32, interval: i32, actor_ids_json: &str, metrics_json: &str, project: Option<&str>, environment: Option<&str>) -> Result<crate::models::ActorsGetActorMetricsResponse, Error<ActorsMetricsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/actors/metrics/history", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.query(&[("metrics_json", &metrics_json.to_string())]);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ActorsMetricsGetError> = serde_json::from_str(&local_var_content).ok();

...
/// struct for typed errors of method [`regions_recommend`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RegionsRecommendError {
    Status400(crate::models::ErrorBody),
    Status403(crate::models::ErrorBody),
    Status404(crate::models::ErrorBody),
    Status408(crate::models::ErrorBody),
...
pub async fn regions_list(configuration: &configuration::Configuration, project: Option<&str>, environment: Option<&str>) -> Result<crate::models::RegionsListRegionsResponse, Error<RegionsListError>> {
...
pub async fn regions_recommend(configuration: &configuration::Configuration, project: Option<&str>, environment: Option<&str>, lat: Option<f64>, long: Option<f64>) -> Result<crate::models::RegionsRecommendRegionResponse, Error<RegionsRecommendError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/regions/recommend", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RegionsRecommendError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

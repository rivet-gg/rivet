/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as serializers from "../../../index";
import * as Rivet from "../../../../api/index";
import * as core from "../../../../core";

export const CreateServerRuntimeRequest: core.serialization.ObjectSchema<
    serializers.servers.CreateServerRuntimeRequest.Raw,
    Rivet.servers.CreateServerRuntimeRequest
> = core.serialization.object({
    build: core.serialization.string(),
    arguments: core.serialization.list(core.serialization.string()).optional(),
    environment: core.serialization.record(core.serialization.string(), core.serialization.string()).optional(),
});

export declare namespace CreateServerRuntimeRequest {
    export interface Raw {
        build: string;
        arguments?: string[] | null;
        environment?: Record<string, string> | null;
    }
}

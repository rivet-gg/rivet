...
) -> Result<crate::models::RegionsListRegionsResponse, Error<RegionsListError>> {
...
pub async fn regions_recommend(
	configuration: &configuration::Configuration,
	project: Option<&str>,
	environment: Option<&str>,
	lat: Option<f64>,
	long: Option<f64>,
) -> Result<crate::models::RegionsRecommendRegionResponse, Error<RegionsRecommendError>> {
	let local_var_configuration = configuration;
	...
	if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
		...
		let local_var_entity: Option<RegionsRecommendError> =
			serde_json::from_str(&local_var_content).ok();
		let local_var_error = ResponseContent {
			status: local_var_status,
			content: local_var_content,

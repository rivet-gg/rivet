...
pub async fn group_join_requests_create_join_request(
	configuration: &configuration::Configuration,
	group_id: &str,
) -> Result<(), Error<GroupJoinRequestsCreateJoinRequestError>> {
	...
	if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
		...
		let local_var_entity: Option<GroupJoinRequestsCreateJoinRequestError> =
...
pub async fn group_join_requests_resolve_join_request(
	configuration: &configuration::Configuration,
	group_id: &str,
	identity_id: &str,
	group_resolve_join_request_request: crate::models::GroupResolveJoinRequestRequest,
) -> Result<(), Error<GroupJoinRequestsResolveJoinRequestError>> {
	let local_var_configuration = configuration;
	...
	local_var_req_builder = local_var_req_builder.json(&group_resolve_join_request_request);
	...
	if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
		...
		let local_var_entity: Option<GroupJoinRequestsResolveJoinRequestError> =
			serde_json::from_str(&local_var_content).ok();
		let local_var_error = ResponseContent {
			status: local_var_status,
			content: local_var_content,

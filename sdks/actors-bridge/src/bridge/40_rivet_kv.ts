...
async function getBatch(
  ...
  let deserializedValues = new Map<string, any>();
 ...
 * @returns {Promise<Map<string, any>>} The retrieved values.
 */
async function list(options?: ListOptions): Promise<Map<string, any>> {
  // Build query
  let query;
  if (options?.prefix) {
    query = {
  ...
  let entries = await op_rivet_kv_list(
    query,
...
async function putBatch(
  obj: Record<any, any | ArrayBuffer>,
  options?: PutBatchOptions,
): Promise<void> {
  let serializedObj = new Map<string, any>();
  let format = options?.format ?? "value";
...
function serializeKey(key: any): any {
...
function serializeListKey(key: any): any[] {
  if (key instanceof Array) {
...
function deserializeKey(key: any): any {
  if ("inKey" in key || "outKey" in key) {
    let jsKey = key.inKey ?? key.outKey;

    let tuple = jsKey.map((x: any) => core.deserialize(x));

...
func (c *Client) Create(ctx context.Context, request *cluster.CreateRequest) (*cluster.CreateResponse, error) {
	baseURL := "https://api.rivet.gg"
	if c.baseURL != "" {
		baseURL = c.baseURL
	}
	endpointURL := baseURL + "/" + "cluster/create"
	...
	var response *cluster.CreateResponse
	if err := c.caller.Call(
		ctx,
		&core.CallParams{
			URL:          endpointURL,

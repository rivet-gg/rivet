...
While comparing benchmarks to Postgres is not quite a fair comparison, it's a _close enough_ comparison to Cockroach's performance given they have similar design constraints. More work should be done to compare the two head to head.
...
ClickHouse also allows us to use the same migration tools (`go-migrate`) for both Cockroach and ClickHouse. Ingesting data in to both are done in similar methods, so our architecture is kept simple and consistent.

...
export default class Example extends Actor {
    ...
    _calcFibonacci(n: number): number {
        if (n <= 1) return n;
        return this._calcFibonacci(n - 1) + this._calcFibonacci(n - 2);
    }

    // This is public and can be called by clients
    fetchFibonacci(n: number): number {
        return this._calcFibonacci(n);
    }
}
```

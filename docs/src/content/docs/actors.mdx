...
Actors are a simple - yet powerful - architecture. Think of actors as independent workers that:
...
- Communicate only through remote function calls - no shared state
...
- Replace the need for complex infrastructure (caches, queues, pubsub) with actors - a simpler architecture
- Improve performance by combining compute (i.e. "functions") with data (i.e. "state")
...
- Make large applications more maintainable with clear system boundaries

## Core concepts

- **Functions** (i.e. remote procedure calls) are how clients communicate with actors & how actors communicate with each other.
...
| **Scaling**             | Need external services (load balancers, databases) to scale         | Scale by creating more instances                                 |
...
| **Performance**         | Performance can be limited by network and database latency          | Optimized performance by co-locating compute and state           |

...
| **Scaling**             | Need external services (load balancers, databases) to scale                  | Cheaper and automatically distribute load                        |
...
| **Performance**         | Performance can be limited by network latency and complex locking mechanisms | Optimized performance by co-locating compute and state           |
| **Latency**             | Typically higher latency due to centralized data centers and databases       | Lower latency by deploying at the edge, closer to users          |

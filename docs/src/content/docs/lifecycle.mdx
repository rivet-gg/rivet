...
Called whenever a new client connects to the actor. Clients can pass parameters when connecting for this function.

Connections will not be able to interact with the actor until this function succeeds. Throwing an error will abort the connection. This can be used for authentication, see [here](/docs/authentication).

The returned value will be associated with the connection and can be accessed later under `conn.data`.

# JavaScript & TypeScript

This guide will walk you through setting up JavaScript/TypeScript client & connect to an actor.

<Warning>
    Make sure you've already deployed a Rivet Actor. If you haven't, follow the [setup guide](/docs/setup) first.
</Warning>

## Install client package

Install the Rivet actor package with:

<CodeGroup>
```sh {{ "title": "npm" }}
npx jsr add @rivet-gg/actor-client
```

```sh {{ "title": "Yarn" }}
yarn dlx jsr add @rivet-gg/actor-client
```

```sh {{ "title": "pnpm" }}
pnpm dlx jsr add @rivet-gg/esbuild-deno-loader
```

```sh {{ "title": "Deno" }}
deno add jsr:@rivet-gg/actor-client
```

```sh {{ "title": "Bun" }}
bunx jsr add @rivet-gg/esbuild-deno-loader
```
</CodeGroup>

<Info>
    These commands don't use the usual `npm install`/`yarn add`/etc since `@rivet-gg/actor-client` package is hosted on [jsr.io](https://jsr.io/docs/why), a portable alternative to NPM.
</Info>

## Connect to actor

Fetch the connection address using the Rivet CLI. Copy this value.

```sh
rivet get-address
```

Use this code to create & connect to the actor. Replace `/* CONNECTION ADDRESS */` with the value from the previous step.

```typescript
import Client from "@rivet-gg/actor-client";
const client = new Client(/* CONNECTION ADDRESS */);

const counter = await client.withTags({ name: "counter" });  // Get or create a `counter` actor
const newCount = await counter.increment(5);  // Call the method `increment` on the `counter` actor
console.log("New count", newCount);
```

You can monitor the actor with:

<Tabs>
<Tab title="Hub">
Visit the [Rivet Hub](https://hub.rivet.gg). Click on the actor with the green dot.
</Tab>
<Tab title="CLI">
```sh
rivet actor list
```
</Tab>
</Tabs>

## Next steps

<CardGroup>
    <Card title="What are Actors?" href="/docs/actors" />
    <Card title="RPC" href="/docs/rpc" />
    <Card title="State" href="/docs/events" />
    <Card title="Examples" href="/examples" />
</CardGroup>

...
impl<R> From<aws_smithy_http::result::SdkError<crate::error::VerifyCustomHostnameError, R>>
	for Error
where
	R: Send + Sync + std::fmt::Debug + 'static,
{
	fn from(
		err: aws_smithy_http::result::SdkError<crate::error::VerifyCustomHostnameError, R>,
	) -> Self {
		match err {
			aws_smithy_http::result::SdkError::ServiceError { err, .. } => match err.kind {
				crate::error::VerifyCustomHostnameErrorKind::InternalError(inner) => {
					Error::InternalError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::RateLimitError(inner) => {
					Error::RateLimitError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::ForbiddenError(inner) => {
					Error::ForbiddenError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::UnauthorizedError(inner) => {
					Error::UnauthorizedError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::NotFoundError(inner) => {
					Error::NotFoundError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::BadRequestError(inner) => {
					Error::BadRequestError(inner)
				}
				crate::error::VerifyCustomHostnameErrorKind::Unhandled(inner) => {

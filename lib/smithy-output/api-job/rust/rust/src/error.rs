...
/// Error type for the `Cleanup` operation.
#[non_exhaustive]
...
/// Types of errors that can occur for the `Cleanup` operation.
#[non_exhaustive]
#[derive(std::fmt::Debug)]
...
impl std::fmt::Display for CleanupError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match &self.kind {
			CleanupErrorKind::InternalError(_inner) => _inner.fmt(f),
			CleanupErrorKind::RateLimitError(_inner) => _inner.fmt(f),
			CleanupErrorKind::ForbiddenError(_inner) => _inner.fmt(f),
			CleanupErrorKind::UnauthorizedError(_inner) => _inner.fmt(f),
			CleanupErrorKind::NotFoundError(_inner) => _inner.fmt(f),
			CleanupErrorKind::BadRequestError(_inner) => _inner.fmt(f),
			CleanupErrorKind::Unhandled(_inner) => _inner.fmt(f),
		}
	}
}
impl aws_smithy_types::retry::ProvideErrorKind for CleanupError {
	fn code(&self) -> Option<&str> {
		CleanupError::code(self)
	}
	fn retryable_error_kind(&self) -> Option<aws_smithy_types::retry::ErrorKind> {
		match &self.kind {
			CleanupErrorKind::InternalError(inner) => Some(inner.retryable_error_kind()),
			CleanupErrorKind::UnauthorizedError(inner) => Some(inner.retryable_error_kind()),
			_ => None,
		}
	}
}
impl CleanupError {
	/// Creates a new `CleanupError`.
	pub fn new(kind: CleanupErrorKind, meta: aws_smithy_types::Error) -> Self {
		Self { kind, meta }
	}

	/// Creates the `CleanupError::Unhandled` variant from any error type.
	pub fn unhandled(err: impl Into<Box<dyn std::error::Error + Send + Sync + 'static>>) -> Self {
		Self {
			kind: CleanupErrorKind::Unhandled(err.into()),
			meta: Default::default(),
		}
	}

	/// Creates the `CleanupError::Unhandled` variant from a `aws_smithy_types::Error`.
	...
	/// Returns `true` if the error kind is `CleanupErrorKind::InternalError`.
	pub fn is_internal_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::InternalError(_))
	}
	/// Returns `true` if the error kind is `CleanupErrorKind::RateLimitError`.
	pub fn is_rate_limit_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::RateLimitError(_))
	}
	/// Returns `true` if the error kind is `CleanupErrorKind::ForbiddenError`.
	pub fn is_forbidden_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::ForbiddenError(_))
	}
	/// Returns `true` if the error kind is `CleanupErrorKind::UnauthorizedError`.
	pub fn is_unauthorized_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::UnauthorizedError(_))
	}
	/// Returns `true` if the error kind is `CleanupErrorKind::NotFoundError`.
	pub fn is_not_found_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::NotFoundError(_))
	}
	/// Returns `true` if the error kind is `CleanupErrorKind::BadRequestError`.
	pub fn is_bad_request_error(&self) -> bool {
		matches!(&self.kind, CleanupErrorKind::BadRequestError(_))
	}
}
impl std::error::Error for CleanupError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match &self.kind {
			CleanupErrorKind::InternalError(_inner) => Some(_inner),
			CleanupErrorKind::RateLimitError(_inner) => Some(_inner),
			CleanupErrorKind::ForbiddenError(_inner) => Some(_inner),
			CleanupErrorKind::UnauthorizedError(_inner) => Some(_inner),
			CleanupErrorKind::NotFoundError(_inner) => Some(_inner),
			CleanupErrorKind::BadRequestError(_inner) => Some(_inner),
			CleanupErrorKind::Unhandled(_inner) => Some(_inner.as_ref()),
		}
	}
}

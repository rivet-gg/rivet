...
impl Client {
	...
	pub async fn with_bucket_and_endpoint(
		...
		let endpoint = match endpoint_kind {
			EndpointKind::Internal => s3_config.endpoint.internal.clone(),
			EndpointKind::InternalResolved => {
				let mut endpoint = s3_config.endpoint.internal.clone();
			...
			EndpointKind::External => s3_config.endpoint.external.clone(),
		};
...
pub fn namespaced_bucket_name(
	...
	Ok(format!("{}-{}", config.rivet.namespace, name))
}

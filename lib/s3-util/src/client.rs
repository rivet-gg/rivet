...
pub fn s3_region(svc_name: &str) -> Result<String, ClientError> {
	let svc_screaming = svc_name.to_uppercase().replace("-", "_");

	std::env::var(format!("S3_REGION_{}", svc_screaming)).map_err(Into::into)
}

pub fn s3_credentials(svc_name: &str) -> Result<(String, String), ClientError> {
	let svc_screaming = svc_name.to_uppercase().replace("-", "_");
...
pub fn s3_endpoint_external(svc_name: &str) -> Result<String, ClientError> {
	let svc_screaming = svc_name.to_uppercase().replace("-", "_");

	std::env::var(format!("S3_ENDPOINT_EXTERNAL_{}", svc_screaming)).map_err(Into::into)
}

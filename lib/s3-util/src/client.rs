...
impl Client {
	...
	pub async fn with_bucket_and_endpoint(
		...
		let endpoint = match endpoint_kind {
			EndpointKind::Internal => s3_config.enpdoint_intenral.clone(),
			EndpointKind::InternalResolved => {
				let mut endpoint = &s3_config.endpoint_intenral;
			...
			EndpointKind::External => s3_config.endpoint_external.clone(),
		};
...
pub fn namespaced_bucket_name(config: &rivet_config::Config, name: &str) -> String {
	format!("{}-{}", config.rivet.namespace, name)
}

...
impl ServiceContextData {
	...
	) -> Result<Vec<(Vec<String>, config::project::service::Secret)>> {
		todo!("secrets");
...
async fn add_s3_env(
	...
	//
	// env.insert(
	// 	format!("S3_BUCKET_{s3_dep_name}"),
	// 	s3_dep.s3_bucket_name().await,
	// );
	// env.insert(
	// 	format!("S3_ENDPOINT_INTERNAL_{s3_dep_name}"),
	...
	//
	// 	env.insert(
	// 		format!("S3_ENDPOINT_EXTERNAL_{s3_dep_name}",),
	// 		external_endpoint,
	// 	);
	// }
	// env.insert(
	// 	format!("S3_REGION_{s3_dep_name}"),
	// 	s3_config.region,
	// );
...
async fn add_s3_secret_env(
	...
	env.insert(
		format!("S3_upper_ACCESS_KEY_ID_{s3_dep_name}"),
		s3_creds.access_key_id,
	);
	env.insert(
		format!("S3_upper_SECRET_ACCESS_KEY_{s3_dep_name}"),

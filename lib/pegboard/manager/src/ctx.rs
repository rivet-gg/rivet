...
impl Ctx {
	...
	pub async fn start(
		...
		// Start runner TCP listener
		let self2 = self.clone();
		tokio::spawn(async move {
			let listener = TcpListener::bind(("127.0.0.1", RUNNER_PORT)).await?;

			loop {
				let (mut socket, _) = listener.accept().await?;

				if let Some(runner) = &*self2.isolate_runner.read().await {
					runner.attach_tcp(socket).await?;
				} else {
					socket.shutdown().await?;
					tracing::error!("no isolate runner to attach socket to");
				}
			}

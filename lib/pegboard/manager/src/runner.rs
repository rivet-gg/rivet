...
impl Handle {
	...
	pub async fn attach_tcp(&self, new_stream: TcpStream) -> Result<()> {
		match &self.comms {
			Comms::Basic => bail!("attempt to attach TCP stream to basic runner"),
			Comms::Tcp(stream) => {
	...
	pub async fn send(&self, packet: &runner_protocol::ToRunner) -> Result<()> {
		match &self.comms {
			Comms::Basic => bail!("attempt to send TCP message to basic runner"),
			Comms::Tcp(stream) => {
				...
				let mut guard = loop {
					...
					attempts += 1;
					if attempts > 15 {
						bail!("timed out waiting for runner socket to open");
					}
...
impl Handle {
	...
	pub fn has_tcp_comms(&self) -> bool {
		matches!(self.comms, Comms::Tcp(_))
	}
}
...
impl Comms {
	pub fn tcp() -> Self {
		Comms::Tcp(Arc::new(Mutex::new(None)))
	}
}

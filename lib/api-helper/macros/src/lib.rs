			...
			if let Some(rate_limit_rate) = self.args.iter().find(|arg| arg.label == "rate_limit") {
				...
				quote! {
					rivet_cache::RateLimitConfig {
						key: format!("{}-{}", std::env::var("CHIRP_SERVICE_NAME").unwrap_or_else(|_| "unknown".to_string()), #key),
						buckets: #buckets,
					}
				}
			} else {
				quote! {
					rivet_cache::RateLimitConfig {
						key: format!("{}{}", std::env::var("CHIRP_SERVICE_NAME").unwrap_or_else(|_| "unknown".to_string()), #default_key),

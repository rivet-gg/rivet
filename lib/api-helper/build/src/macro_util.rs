...
use rivet_config::config::DnsProvider;
...
impl __RouterConfig {
	pub fn new(config: &rivet_config::Config, uri: &hyper::Uri) -> GlobalResult<Self> {
		...
		let url = format!("{}{}", config.server()?.rivet.origin.api, uri);
...
pub async fn __with_ctx<A: auth::ApiAuth + Send>(
	...
	let remote_address = if internal_endpoint {
		...
		let remote_address_str =
			if config.server()?.rivet.dns_provider == Some(DnsProvider::Cloudflare) {
	...
	let svc_name = std::env::var("CHIRP_SERVICE_NAME").unwrap();

...
pub async fn __with_ctx<A: auth::ApiAuth + Send>(
	...
	let remote_address = if internal_endpoint {
		...
		// Decode remote address. We use the x-forwarded-for generated by
		// Traefik if this is exposed directly to the internet. Otherwise, we
		// use the cf-connecting-ip header since that's the recommended header
		// to use by Cloudflare.
		let remote_address_str = if not_using_cloudflare {

...
async function copyFiles(
	...
	await $.logGroup(async () => {
		...
		await awsCommand.command(
			`aws s3 rm s3://rivet-releases/${targetPrefix} --recursive`,
		).spawn();
...
async function generateInstallScripts(
	...
	for (const scriptPath of installScriptPaths) {
		...
		await awsCommand.command(`aws s3 cp - s3://rivet-releases/${uploadKey}`)
			.stdinText(scriptContent).spawn();
	}
}

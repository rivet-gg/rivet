...
pub async fn portal_notifications_register_notifications(configuration: &configuration::Configuration, portal_register_notifications_request: crate::models::PortalRegisterNotificationsRequest) -> Result<(), Error<PortalNotificationsRegisterNotificationsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/portal/notifications/register", local_var_configuration.base_path);
    ...
    local_var_req_builder = local_var_req_builder.json(&portal_register_notifications_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<PortalNotificationsRegisterNotificationsError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn portal_notifications_unregister_notifications(configuration: &configuration::Configuration, service: &str) -> Result<(), Error<PortalNotificationsUnregisterNotificationsError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<PortalNotificationsUnregisterNotificationsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
